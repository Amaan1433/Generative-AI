...
!pip install weaviate-client[text2vec-contextionary]
...
from weaviate.contextionary import Contextionary # Import Contextionary from weaviate.contextionary
...
model_name = "all-mpnet-base-v2"  # Replace with your desired model
...
embeddings = model.encode(str(names_dict))
print(embeddings)
...
result = res.query.hybrid(
    vector = embeddings, # Pass the vector to the 'vector' parameter
    query="ethical",
    limit=5,
    query_properties=["name", "filename", "content"]
)
...
result = res.generate.near_text(
    query = "Remote work",
    #distance = "cosine",
    target_vector = prompt,
    limit = 5,
    #single_prompt= "Translate to french: {query}",
    grouped_task=prompt,
    grouped_properties=["name", "filename", "content"]
)
for item in result.objects:
  print(item)
